...
export default function RachaTiempoScreen({ navigation }) {
  ...
  useEffect(() => {
    if (gameOver || isAnswering || !prepared) return;
  ...
  }, [currentIndex, gameOver, isAnswering, prepared]);
  ...
  const handleAnswer = async (isCorrect, correctText) => {
    ...
    if (isCorrect) {
      ...
      setStreak(newStreak);
      await addPoints(15); // More points for timed mode
      await markStudyDay();
      await updateStreak();
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.gameOverContainer}>
          <View style={styles.gameOverIcon}>
            <Ionicons
              name={gameOverReason === 'timeout' ? 'timer-outline' : 'flame'}
              size={64}
              color={gameOverReason === 'timeout' ? Colors.error : Colors.secondary}
            />
          </View>
          <Text style={styles.gameOverTitle}>
            {gameOverReason === 'timeout' ? '¡Se acabó el tiempo!' : '¡Racha terminada!'}
          </Text>
          <Text style={styles.gameOverStreak}>{streak}</Text>
          <Text style={styles.gameOverLabel}>preguntas correctas consecutivas</Text>

          <View style={[styles.gameOverInfoCard, gameOverReason === 'timeout' && { backgroundColor: '#FFF3E0' }]}>
            <Ionicons
              name={gameOverReason === 'timeout' ? 'timer-outline' : 'close-circle'}
              size={20}
              color={gameOverReason === 'timeout' ? '#E65100' : Colors.error}
            />
            <Text style={styles.gameOverInfoText}>
              {gameOverReason === 'timeout'
                ? `Tardaste más de ${TIME_LIMIT} segundos.\n`
                : ''}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={() => { clearInterval(timerRef.current); navigation.goBack(); }} style={styles.closeBtn}>
          <Ionicons name="close" size={24} color={Colors.textPrimary} />
        </TouchableOpacity>
        <Text style={styles.headerTitle}>Racha por Tiempo</Text>
      ...
      <TimerBar timeLeft={timeLeft} totalTime={TIME_LIMIT} />
